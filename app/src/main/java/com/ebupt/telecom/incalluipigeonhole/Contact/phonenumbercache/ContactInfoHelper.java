...
package com.ebupt.telecom.incalluipigeonhole.Contact.phonenumbercache;
...
import com.ebupt.telecom.incalluipigeonhole.Contact.ContactSource;
import com.ebupt.telecom.incalluipigeonhole.Contact.common.Constants;
import com.ebupt.telecom.incalluipigeonhole.Contact.common.ContactsUtils;
import com.ebupt.telecom.incalluipigeonhole.base.Log;
import com.ebupt.telecom.incalluipigeonhole.compat.PhoneNumberUtilsCompat;
import com.ebupt.telecom.incalluipigeonhole.utils.Assert;
import com.ebupt.telecom.incalluipigeonhole.utils.PermissionsUtil;
import com.ebupt.telecom.incalluipigeonhole.utils.PhoneNumberHelper;
import com.ebupt.telecom.incalluipigeonhole.utils.TelecomUtil;
import com.ebupt.telecom.incalluipigeonhole.utils.UriUtils;
...
public class ContactInfoHelper {
  ...
  public ContactInfo lookupNumber(String number, String countryIso, long directoryId) {
    if (TextUtils.isEmpty(number)) {
      Log.d("ContactInfoHelper.lookupNumber", "number is empty");
      return null;
    }
    ...
    if (PhoneNumberHelper.isUriNumber(number)) {
      Log.d("ContactInfoHelper.lookupNumber", "number is sip");
    ...
    if (info == null) {
      // The lookup failed.
      Log.d("ContactInfoHelper.lookupNumber", "lookup failed");
      updatedInfo = null;
    } else {
  ...
  ContactInfo lookupContactFromUri(Uri uri) {
    if (uri == null) {
      Log.d("ContactInfoHelper.lookupContactFromUri", "uri is null");
      return null;
    }
    if (!PermissionsUtil.hasContactsReadPermissions(context)) {
      Log.d("ContactInfoHelper.lookupContactFromUri", "no contact permission, return empty");
    ...
    try (Cursor phoneLookupCursor =
      ...
      if (phoneLookupCursor == null) {
        Log.d("ContactInfoHelper.lookupContactFromUri", "phoneLookupCursor is null");
        return null;
      }
...
//              phoneLookupCursor, PhoneQuery.MATCHED_NUMBER, uri);
      UriUtils.areEqual(uri, Contacts.getLookupUri(PhoneQuery.MATCHED_NUMBER, phoneLookupCursor.getString(PhoneQuery.MATCHED_NUMBER)));
      if (!hasNumberMatch) {
        return ContactInfo.EMPTY;
      }
  ...
  private ContactInfo queryContactInfoForPhoneNumber(
    ...
    if (TextUtils.isEmpty(number)) {
      Log.d("ContactInfoHelper.queryContactInfoForPhoneNumber", "number is empty");
      return null;
    }

    ContactInfo info = lookupContactFromUri(getContactInfoLookupUri(number, directoryId));
    if (info == null) {
      Log.d("ContactInfoHelper.queryContactInfoForPhoneNumber", "info looked up is null");
    }
    if (info != null && info != ContactInfo.EMPTY) {
      ...
      if (cacheInfo != null) {
        if (!cacheInfo.getContactInfo().isBadData) {
          ...
          Log.i("ContactInfoHelper.queryContactInfoForPhoneNumber", "info is bad data");
        }
      }
    }
    return info;
  ...
  private String formatPhoneNumber(String number, String normalizedNumber, String countryIso) {
    ...
    return PhoneNumberUtilsCompat.formatNumber(number, normalizedNumber, countryIso);
  }
  ...
  public void updateCachedNumberLookupService(ContactInfo updatedInfo) {
    if (cachedNumberLookupService != null) {
      if (hasName(updatedInfo)) {
        CachedNumberLookupService.CachedContactInfo cachedContactInfo =
