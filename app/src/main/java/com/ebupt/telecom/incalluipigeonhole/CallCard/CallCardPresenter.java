...
package com.ebupt.telecom.incalluipigeonhole.CallCard;
...
import com.ebupt.telecom.incalluipigeonhole.incalluibind.ObjectFactory;
...
public class CallCardPresenter extends Presenter<CallCardPresenter.CallCardUi>
        implements InCallPresenter.InCallStateListener, InCallPresenter.IncomingCallListener, InCallPresenter.InCallDetailsListener,
        InCallPresenter.InCallEventListener, CallList.CallUpdateListener {
    ...
    public void onStateChange(InCallPresenter.InCallState oldState, InCallPresenter.InCallState newState, CallList callList) {
        ...
        } else if (newState == InCallPresenter.InCallState.PENDING_OUTGOING || newState == InCallPresenter.InCallState.OUTGOING) {
    ...
    private void updateContactEntry(ContactInfoCache.ContactCacheEntry entry, boolean isPrimary) {
        if (isPrimary) {
     ...
     */
    private static String getNumberForCall(ContactInfoCache.ContactCacheEntry contactInfo) {
    ...
    private void maybeSendAccessibilityEvent(InCallPresenter.InCallState oldState, InCallPresenter.InCallState newState) {
        if (mContext == null) {
            return;
        }
        ...
        if ((oldState != InCallPresenter.InCallState.OUTGOING && newState == InCallPresenter.InCallState.OUTGOING)
                || (oldState != InCallPresenter.InCallState.INCOMING && newState == InCallPresenter.InCallState.INCOMING)) {
            if (getUi() != null) {
